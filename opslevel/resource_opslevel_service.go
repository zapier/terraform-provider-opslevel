...
func resourceServiceCreate(d *schema.ResourceData, client *opslevel.Client) error {
	...
	if ok1 && ok2 {
		_, err := client.ServiceApiDocSettingsUpdate(resource.Id.(string), docPath.(string), docSource.(opslevel.ApiDocumentSourceEnum))
		if err != nil {
			return err
		}
	}
...
func resourceServiceUpdate(d *schema.ResourceData, client *opslevel.Client) error {
	...
	if d.HasChange("api_document_path") || d.HasChange("preferred_api_document_source") {
		docPath := d.Get("api_document_path").(string)
		docSource := d.Get("preferred_api_document_source").(opslevel.ApiDocumentSourceEnum)
